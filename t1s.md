...
edge in $T$ but not in $T'$. The Greedy Swap Lemma implies that
$T' \cup \{e\}$ contains a single cycle $C$ and that removing any other
edge from the cycle yields a spanning tree. Since $T'$ is an MST, we get
that $w_f \leq w_e$ for every other edge $f$ in $C$; otherwise, we can
swap $f$ with $e$ and get a cheaper spanning tree.
